...
class ConvertCsvToApi
{
     ...
     */
    public function parse_query($query = null, $csv_name)
    {
     ...
     */
    public function get_extension($source = null)
    {
        if ($source == null) {
            $source = $this->source;
        }
     ...
     */
    public function normalize_line_endings($string)
    {
        $string = str_replace("\r\n", "\n", $string);
     ...
     */
    public function object_to_json($data)
    {
     ...
     */
    public function header($extension = null)
    {
        if ($extension == null) {
     ...
     * settings.
     */
    public function query($data, $query = null)
    {
        if ($query == null) {
            $query = $_GET;
        }
     ...
     */
    public function get_query_vars($data)
    {
        $vars = array();
        foreach ($data as $row) {
            foreach ($row as $key=>$value) {
     ...
     *
     */
    public function kses_bad_protocol_once($string, $allowed_protocols, $count = 1)
    {
     ...
     *
     */
    public function default_vars($pairs, $atts)
    {
        $atts = (array)$atts;
        $out = array();
